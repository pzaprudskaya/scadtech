...
export class FeedbackIdComponent implements OnInit {
  ...
  ngOnInit() {
    this.feedbackService.getFeedback(this.route.snapshot.params.id).subscribe((feedBack) => {
      this.feedback = feedBack[0];
      this.feedback.unread = false;
      this.feedbackService.updateFeedback(this.feedback).subscribe(() => console.log('Update'));
    });
  }
