...
export class DocumentService {
  ...
  getDocuments(viewPages, skipPages): Observable<IAllDocuments> {
    ...
    return this.http.get<IAllDocuments>(this.API_URL, httpOptions).pipe(
      tap((data: IAllDocuments) => console.log('Documents: ' + JSON.stringify(data))),
      catchError(this.handleError)
    );
  }

  getDocument(id: string): Observable<IDocument> {
    return this.http.get<IDocument>(`${this.API_URL}/${id}`, this.httpOptions).pipe(
      tap((data: IDocument) => console.log('Document: ' + JSON.stringify(data))),
      catchError(this.handleError)
    );
  }

  addDocument(value: IDocument) {
    return this.http.post<IDocument>(this.API_URL, JSON.stringify(value), this.httpOptions).pipe(
      tap(addDocument => console.log('Add document: ' + JSON.stringify(addDocument))),
      catchError(this.handleError));
  }
  ...
  deleteDocument(value: IDocument) {
    return this.http.delete<void>(`${this.API_URL}/${value._id}`, this.httpOptions).pipe(
      tap(deleteDocument => console.log('Delete document: ' + JSON.stringify(deleteDocument))),
      catchError(this.handleError));
  }
